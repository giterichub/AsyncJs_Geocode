...
const getCoordinates = function (latitudeValue, longitudeValue){
    fetch(`https://geocode.xyz/${latitudeValue},${longitudeValue}?geoit=json&auth=12136512126515e15934668x72484`)
    .then(response => response.json())
    .then(data => {
        console.log(`You are in ${!(data.state.length) ? 'a state is not specified' : data.state}, ${data.country}`);
        return data.country
    })
    .then(response => {
        return fetch(`https://restcountries.com/v3.1/name/${response}`)
    })
    .then(response => {
